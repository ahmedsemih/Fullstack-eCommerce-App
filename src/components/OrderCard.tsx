  ...
  return (
    ...
    >
        <div className='text-xl md:flex-row flex-col flex items-center justify-between pb-4 px-2'>
            <p>Order Date: <i className='font-semibold'>{moment(order.orderDate).format('DD.MM.YYYY')}</i></p>
            {
                order.deliveryDate && <p>Deliver Date: <i className='font-semibold'>{moment(order.deliveryDate).format('DD.MM.YYYY')}</i></p>
            }
